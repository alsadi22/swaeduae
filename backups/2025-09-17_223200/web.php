...
Route::redirect('/email/verify','/verify-email',302);
// == Volunteer Social Login (Google) ==
...
Route::get('/auth/google/callback', function () {
    try {
        $gUser = \Laravel\Socialite\Facades\Socialite::driver('google')->stateless()->user();
    } catch (\Throwable $e) {
        return redirect('/login')->withErrors(['oauth' => 'Google sign-in failed.']);
    }

    $user = \App\Models\User::firstOrCreate(
        ['email' => $gUser->getEmail()],
        [
            'name'     => $gUser->getName() ?: (explode('@', $gUser->getEmail())[0] ?? 'Volunteer'),
...
// == Volunteer login view override (main site) ==
Route::middleware(['web','guest'])->get('/login', fn() => view('auth.login'))->name('login');
// == Auth route synonyms (UI uses "Sign In / Sign Up")
Route::redirect('/signin','/login',302);
Route::redirect('/sign-in','/login',302);
Route::redirect('/signup','/register',302);
Route::redirect('/sign-up','/register',302);
