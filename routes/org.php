...
Route::get('/org/login', fn () => view(view()->exists('tmp.org_login_min') ? 'tmp.org_login_min' : (view()->exists('organization.login') ? 'organization.login' : 'auth.organization_login')))->name('org.login');
Route::post('/org/login', [\App\Http\Controllers\Auth\LoginController::class, 'login'])->name('org.login.perform');
