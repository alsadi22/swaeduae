...
class AttendanceAdminController extends Controller
{
    ...
    {
        $oid = (int) $request->query('opportunity_id', 0);

        $rows = DB::table('attendances as a')
            ->join('users as u', 'u.id', '=', 'a.user_id')
            ->when($oid > 0, fn ($q) => $q->where('a.opportunity_id', $oid))
            ->leftJoin('certificates as c', function ($join) {
                $join->on('c.user_id', '=', 'a.user_id')
                     ->on('c.event_id', '=', 'a.opportunity_id');
            })
            ->orderByDesc('a.id')
            ->selectRaw('a.*, u.name, u.volunteer_code, c.id as certificate_id, c.file_path as certificate_file')
            ->paginate(30)
            ->withQueryString();
    ...
    public function manual(Request $request, int $opportunityId)
    {
        ...
        $row = DB::table('attendances')->where([
            'user_id' => $u,
    ...
    public function complete(Request $request, int $opportunityId)
    {
        ...
        $n = HoursService::finalize($opportunityId);
        $c = CertificateService::issueForOpportunity($opportunityId);

        return back()->with(
            'status',
            __("Completed. Finalized hours: :n, Certificates issued: :c", ['n' => $n, 'c' => $c])
        );
    }
}
